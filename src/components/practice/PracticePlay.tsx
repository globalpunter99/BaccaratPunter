...
export default function PracticePlay() {
  ...
  function placeGuess(outcome: Outcome) {
    if (revealed) return;
    setPendingGuess(outcome);
  }
  ...
  function revealResult() {
    ...
    setGuesses(prev => {
      const next = [...prev];
      next[currentIdx] = { ...next[currentIdx], guess: pendingGuess, revealed: true };
      return next;
    });
    setRevealed(true);
  }
  ...
  function nextHand() {
    if (currentIdx + 1 >= guesses.length) {
      setPhase("done");
      return;
    }
    setCurrentIdx(i => i + 1);
    ...
    return (
      <div className="page" style={{ maxWidth: 700 }}>
        ...
        <div style={{ color: "var(--text-secondary)", fontSize: 13, marginBottom: 20 }}>
          Select a session from your library. The results will be hidden — play through it hand by hand
          as if you're at the table. After placing your guess, reveal the actual result.
      ...
      <div className="page" style={{ maxWidth: 500 }}>
        ...
        <div className="panel" style={{ textAlign: "center", padding: 40 }}>
          ...
          <div className="flex gap-8" style={{ justifyContent: "center" }}>
            <button className="btn btn-gold" onClick={() => setPhase("pick")}>Play Another</button>
            <button className="btn btn-secondary" onClick={() => startSession(sessionId)}>Replay Same</button>
          </div>
        </div>
      </div>
    );
  ...
  // ── Active play ──────────────────────────────────────────────────────────
  const actual = currentGuess?.actual;
    ...
    <div className="page">
      <div className="flex items-center justify-between mb-12">
        <div>
          ...
          <div style={{ fontSize: 13, color: "var(--text-secondary)" }}>
            {session?.venue} · Hand {currentIdx + 1} of {guesses.length}
          </div>
        </div>
        <div className="flex gap-8 items-center">
          ...
          <button className="btn btn-ghost" onClick={() => setPhase("pick")}>End Practice</button>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: 16 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div className="panel">
            <div className="panel-title">Hand {currentIdx + 1}</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {!revealed ? (
                <>
                ...
                <>
                  ...
                  <div style={{
                    ...
                    <div style={{
                      ...
                      {actual === "banker" ? "BANKER" : actual === "player" ? "PLAYER" : "TIE"}
                    </div>
                    {pendingGuess && (
                  ...
                  <button className="btn btn-gold" onClick={nextHand}>
                    {currentIdx + 1 >= guesses.length ? "Finish" : "Next Hand →"}
                  </button>
                </>
              )}
            </div>
          ...
          <div className="panel">
            ...
            <div className="grid-3">
              <div className="stat-block">
                <div className="stat-value">{currentIdx + 1}</div>
        ...
        <div>
          {revealedOutcomes.length === 0 ? (
            <div className="panel" style={{ textAlign: "center", padding: 40 }}>
              <div style={{ fontSize: 14, color: "var(--text-muted)" }}>
                Roads will appear as hands are revealed
              </div>
            </div>
          ) : (
            <RoadsDisplay outcomes={revealedOutcomes} />
          )}
        </div>
      </div>
    </div>
